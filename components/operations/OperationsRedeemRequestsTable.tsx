...
interface Props {
  ...
  onProcess: (row: RedeemRequestRow) => void;
}
...
export function OperationsRedeemRequestsTable({ rows, loading, emptyMessage = "No redeem requests.", busyId, onView, onProcess }: Props) {
  if (loading) {
    return (
    ...
    <div className="overflow-x-auto rounded-xl border border-gray-700 bg-slate-800/50">
      <table className="min-w-full divide-y divide-gray-700 text-left text-sm">
        ...
        <tbody className="divide-y divide-gray-700 bg-slate-800/30">
          {rows.map((r) => {
            const canProcess = r.operations_status === "pending";
                  ...
                  <div className="flex justify-end gap-2">
                    ...
                    {canProcess && (
                      <button
                        ...
                        onClick={() => onProcess(r)}
                        disabled={rowBusy}
                        className="inline-flex h-9 w-9 items-center justify-center rounded-lg bg-blue-600 text-white hover:bg-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500 disabled:opacity-50"
                        title="Process"
                      >
                        {rowBusy ? <Loader2 className="h-4 w-4 animate-spin" /> : <PlayCircle className="h-4 w-4" />}
                      </button>
                    )}
                  </div>
                </td>
