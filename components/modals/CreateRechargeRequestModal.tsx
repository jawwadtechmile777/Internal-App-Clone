...
interface CreateRechargeRequestModalProps {
  open: boolean;
  onClose: () => void;
  entityId: string;
...
export function CreateRechargeRequestModal({
  open,
  onClose,
  entityId,
  requestedByUserId,
  onCreated,
}: CreateRechargeRequestModalProps) {
  const { showToast } = useToast();
  const { state, actions } = useRechargeForm({
    entityId,
    requestedByUserId,
    onSuccess: () => {
      onCreated();
  ...
  useEffect(() => {
    if (!open) return;
    // reset when opening to avoid stale player selection between entities
    actions.reset();
  ...
  const handleSubmit = async () => {
    ...
    if (ok) {
      showToast({ variant: "success", title: "Recharge request created" });
      onClose();
    } else if (state.submitError) {
      showToast({ variant: "error", title: "Failed to create recharge", description: state.submitError });
    }
  };
    ...
    <Modal open={open} onClose={onClose} title="Create Recharge Request">
      <div className="space-y-4">
            ...
            {canShowSuggestions && (
              <div className="absolute z-10 mt-1 w-full overflow-hidden rounded-lg border border-gray-700 bg-slate-800 shadow-xl">
                {state.playerSearching ? (
                  ...
                  <div className="px-3 py-2 text-sm text-gray-400">No player with this name available</div>
                ) : (
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <div>
            ...
            <select
              ...
              className={`mt-1 w-full rounded-lg border bg-slate-900 px-3 py-2 text-sm text-gray-100 focus:outline-none focus:ring-2 focus:ring-slate-500 disabled:opacity-60 ${
                state.fieldErrors.game ? "border-red-700" : "border-gray-600 focus:border-slate-500"
              }`}
            >
              <option value="">{state.selectedPlayer ? "Select game" : "Select player first"}</option>
              {state.games.map((g) => (
          ...
          <div>
            ...
            <select
              ...
              className={`mt-1 w-full rounded-lg border bg-slate-900 px-3 py-2 text-sm text-gray-100 focus:outline-none focus:ring-2 focus:ring-slate-500 disabled:opacity-60 ${
                state.fieldErrors.paymentMethod ? "border-red-700" : "border-gray-600 focus:border-slate-500"
              }`}
            >
              <option value="">{state.selectedPlayer ? "Select payment method" : "Select player first"}</option>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <div>
            ...
            <input
              ...
              className={`mt-1 w-full rounded-lg border bg-slate-900 px-3 py-2 text-sm text-gray-100 placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-slate-500 ${
                state.fieldErrors.amount ? "border-red-700" : "border-gray-600 focus:border-slate-500"
              }`}
            />
            {state.fieldErrors.amount && <p className="mt-1 text-xs text-red-300">{state.fieldErrors.amount}</p>}
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-300">Bonus Percentage (optional)</label>
            <input
              type="number"
              min="0"
              step="0.01"
              value={state.bonusPercentage}
              onChange={(e) => actions.setBonusPercentage(e.target.value)}
          ...
          <div>
            <label className="block text-sm font-medium text-gray-300">Bonus Amount (auto / editable)</label>
            <input
              type="number"
              min="0"
              step="0.01"
              value={state.bonusAmount}
              onChange={(e) => actions.setBonusAmount(e.target.value)}
              className="mt-1 w-full rounded-lg border border-gray-600 bg-slate-900 px-3 py-2 text-sm text-gray-100 placeholder-gray-500 focus:border-slate-500 focus:outline-none focus:ring-2 focus:ring-slate-500"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-300">Final Amount (preview)</label>
            <input
              value={formatMoney(state.finalAmountPreview)}
              readOnly
              className="mt-1 w-full rounded-lg border border-gray-700 bg-slate-900 px-3 py-2 text-sm text-gray-200"
            />
          </div>
        </div>
        ...
        <div>
          ...
          <textarea
            value={state.remarks}
            onChange={(e) => actions.setRemarks(e.target.value)}
            rows={2}
            className="mt-1 w-full rounded-lg border border-gray-600 bg-slate-900 px-3 py-2 text-sm text-gray-100 placeholder-gray-500 focus:border-slate-500 focus:outline-none focus:ring-2 focus:ring-slate-500"
          />
        </div>
